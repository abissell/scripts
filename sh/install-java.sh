...
run_with_prompt "wget ${location}.sha256 -P /tmp/jdk-tar"
filename="$(find /tmp/jdk-tar -maxdepth 1 -mindepth 1 | grep -v sha256)"
echo 'Verify checksums:'
sha256sum "${filename}"
cat "${filename}.sha256"
press_enter_to_continue
run_with_prompt 'mkdir -p /tmp/jdk-extracted'
run_with_prompt "tar -zxvf ${filename} --directory /tmp/jdk-extracted"
...
RC
  ...
  if prompt "Parsed current Java version ${current_version}, would you like to replace it with ${jdk_dirname}?"; then
    run_with_prompt "cd ${HOME}/.local/lib/jvm && rm default && ln -s ${jdk_dirname} default && ls -lah . && cd ${HOME}"
  else
